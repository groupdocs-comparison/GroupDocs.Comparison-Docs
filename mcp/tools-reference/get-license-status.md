...
url: comparison/mcp/tools-reference/get-license-status
...
| `server` / `engine` | MCP server version and the underlying GroupDocs.Comparison engine version |
| `note` | Why licensing did not end up where you asked — present only when something needs attention |

A key pair that the engine rejects does not fail silently: the mode reverts to `evaluation` and `note` carries the reason, so you learn it here rather than from a watermark on your result document. See [Licensing]({{< ref "comparison/mcp/getting-started/licensing.md" >}}).

## Example prompts

* *"Is the comparison server licensed?"*
* *"How much metered credit have I used?"*
* *"Which GroupDocs.Comparison engine version is this server running?"*
