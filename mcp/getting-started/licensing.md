---
id: mcp-licensing
url: comparison/mcp/getting-started/licensing
title: Licensing
weight: 7
description: "The GroupDocs.Comparison MCP server runs in evaluation mode out of the box; unlock it with your existing GroupDocs.Comparison license file or with metered (pay-per-use) keys, and confirm the active mode with the get_license_status tool."
...
The GroupDocs.Comparison MCP server runs in **evaluation mode out of the box** — no sign-up, no key. Your existing GroupDocs.Comparison license unlocks full functionality: point the installer's `licensePath` — or the `GROUPDOCS_LICENSE_PATH` environment variable in a manual install — at your `.lic` file.

**The MCP server itself is open source (MIT); the underlying GroupDocs.Comparison engine requires a license for production use.**
...
| **Evaluation** | nothing | Only the first 2 pages are compared, and the result carries a watermark |
...
* The marked-up result document carries an **evaluation watermark**, and tool responses include an evaluation-mode notice.
* Only the **first two pages** of each document are compared, so a long contract silently reports far fewer changes than it has.
...
```json
{ "licensePath": "D:/Storage/Licenses/GroupDocs.Comparison.lic" }
```

**Manual (NuGet channel)** — set the environment variable in your client's server entry:

```json
"env": { "GROUPDOCS_LICENSE_PATH": "/path/to/GroupDocs.Comparison.lic" }
```
...
{{< alert style="warning" >}}
**Both keys are required.** With only one set, the server ignores the metered configuration and stays in evaluation mode — it reports this explicitly rather than failing silently. Check with [`get_license_status`]({{< ref "comparison/mcp/tools-reference/get-license-status.md" >}}).
{{< /alert >}}
...
Ask your agent *"what is the license status of the comparison server?"*. The [`get_license_status`]({{< ref "comparison/mcp/tools-reference/get-license-status.md" >}}) tool (server **26.9.0+**) answers without processing a document:
...
```json
{
  ...
  "server": { "name": "GroupDocs.Comparison.Mcp", "version": "26.9.1" },
  "engine": { "name": "GroupDocs.Comparison", "version": "26.8" }
}
```

If a key pair is rejected, the mode reverts to `evaluation` and the `note` field says why — for example *"Metered keys were supplied but the engine rejected them (Authentication failed.)"* — so a mistyped key surfaces here instead of as a watermark on your result document.

## Getting a license

* [Free 30-day temporary license](https://purchase.groupdocs.com/temporary-license/) — full functionality for evaluation.
* [Purchase GroupDocs.Comparison](https://purchase.groupdocs.com/pricing/comparison/net) · [Product page](https://products.groupdocs.com/comparison/net/)
